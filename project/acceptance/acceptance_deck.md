...
`RiverHawk_IMAX_Design_Evolution_and_Demo.mp4` (1:57) is built for this review. The first
69 seconds show each HCD finding as a **live before/after** — the July build you evaluated
runs, then the delivered build runs on the same thread — so the changes can be judged on
their merits rather than discovered. The remainder is the working product end to end. A
capability-only cut is included separately for audiences who don't need the derivation.
