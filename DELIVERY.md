...
Repository: `Blackfell-Group/imax-chat-viewer` · Release tag: **`v1.2.1-prototype`**
...
> Deliverable ①; `v1.2.1-prototype` closes twelve defects from internal review plus three found
...
> deliverables the executed agreement requires. **Pull `v1.2.1-prototype`.**
...
| ③ | e2e smoke test suite | [`tests/`](tests/) — `npm run test:ng` (**55 specs**) · `npm test` (React reference, 16) · results report in [`project/delivery/test_results.pdf`](project/delivery/test_results.pdf) | Complete |
...
**Import the source with:**

```sh
curl -fsSLO https://github.com/Blackfell-Group/imax-chat-viewer-public/archive/refs/tags/v1.2.1-prototype.tar.gz
tar -xzf v1.2.1-prototype.tar.gz && cd imax-chat-viewer-public-1.2.1-prototype
```

2.2 MB, unauthenticated, source only — 212 files. The mirror was rebuilt on 7 August to
carry code and nothing else: the HCD artifacts, guides, compliance matrix, acceptance deck
and recordings were removed from it, along with its history, because none of them needs an
open route. They are transmitted to the Government directly instead.
...
5. **Walkthrough re-recorded and narrated.** Two cuts:
   [`RiverHawk_IMAX_Walkthrough_2026-08-08.mp4`](project/acceptance/RiverHawk_IMAX_Walkthrough_2026-08-08.mp4)
   (4:05, full) and
   [`..._capability.mp4`](project/acceptance/RiverHawk_IMAX_Walkthrough_2026-08-08_capability.mp4)
   (3:49, without the design derivation). Both reproducible from `narration.json`,
   `scripts/capture-walkthrough.js` and `scripts/build-walkthrough.sh`.
