...
async function main() {
  ...
  // --- 3d. The output -----------------------------------------------------
  await cue('15_promote', '3d', 'A standing channel worked whole, then the careful path');
  // The standing channel first: real volume on screen, scrolled, so the windowed
  // stream is demonstrated against demonstration data rather than asserted. Then
  // the bulk accept that makes a conversation of that size finishable at all.
  await page.getByTestId('thread-t-3000').click();
  await page.getByTestId('chat-viewer').waitFor();
  await beat(1400);
  await page.evaluate(() => {
    const el = document.querySelector('[data-testid="message-stream"]');
    if (el) el.scrollTop = 45_000;
  });
  await beat(1500);
  ...
  // Then the careful path on a thread small enough to read through, which is
  // what actually gets promoted here — a 2,388-message export would fill the
  // dialog with more transcript than anyone can look at on screen.
  await page.getByTestId('thread-t-1002').click();
  await page.getByTestId('chat-viewer').waitFor();
  ...
  // The read on the conversation itself, written before promotion so it leads
  // the gold copy the export shows next.
  await page.getByTestId('thread-note-toggle').click();
  await beat(700);
  await page
    .getByTestId('thread-note-field')
    .fill('Speaker does not read as a native Farsi speaker — calqued phrasing throughout. The two clearly know each other well.');
  await beat(1600);
