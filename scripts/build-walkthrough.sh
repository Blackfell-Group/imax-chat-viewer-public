...
# CUT=capability builds the product-only cut promised in the acceptance deck:
# same footage, narration without the design derivation. Scenes whose text is
# unchanged reuse their existing audio, so a second cut costs only the lines
# that actually differ.
CUT="${CUT:-full}"
if [ "$CUT" = "capability" ]; then
  NARRATION="$ACC/narration.capability.json"
  OUT_SUFFIX="_capability"
else
  NARRATION="$ACC/narration.json"
  OUT_SUFFIX=""
fi
SILENT="$ACC/RiverHawk_IMAX_Walkthrough_${DATE}_silent.mp4"
OUTPUT="$ACC/RiverHawk_IMAX_Walkthrough_${DATE}${OUT_SUFFIX}.mp4"
WORK="$ACC/.build-$DATE"
AUDIO="$ACC/.audio"

for f in "$CUES" "$NARRATION" "$SILENT"; do
