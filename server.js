...
app.listen(PORT, async () => {
  console.log(`imax mock services listening on :${PORT}`);
  // Ask the gateway what it publishes and check our names against it. A model
  // name the gateway has never heard of answers 404 on the first translation,
  // which reads like an outage rather than like a ConfigMap typo; and a vision
  // model left unset means every scan silently falls back to stored text.
  // Both are cheaper to learn here than from an officer.
  try {
    for (const line of await require('./providers/model-gateway').preflight()) console.log(line);
  } catch (err) {
    console.warn(`model gateway preflight skipped: ${err.message}`);
  }
});
