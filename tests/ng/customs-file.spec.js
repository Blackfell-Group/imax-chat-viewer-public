...
test('the customs file opens, pages through all five, and reads RTL', async ({ page }) => {
  ...
  // The direction moved off the <li> and onto the source cell when lines gained
  // their own English column (8 Aug) — `dir="auto"` would have guessed per line
  // and could flip a numeric row like "٤٢ | ٦٣٠٠" the wrong way, so the source
  // script now sets it explicitly for the whole document.
  await expect(blocks.locator('.block-src[dir="rtl"]').first()).toBeVisible();
