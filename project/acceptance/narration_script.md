...
Length: **4:08** with Brian. Brian is *slower* than the macOS fallback, not faster — 249
...
>
> What is not proven: it has not run in the target environment, and the live model gateway
> is wired but unexercised. Both are stated in the acceptance package rather than left to be
> discovered.
...
- **Deliver two cuts.** The full recording, and a capability-only cut without segment 2 for
  audiences who do not need the derivation.
