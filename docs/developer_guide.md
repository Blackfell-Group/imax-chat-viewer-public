...
deliverable the Government maintains.

`itemSizePx` is **81**, and that number is measured rather than chosen: bubbles render at
67px (English) or 94px (foreign, which carries the translation line). It matters more than
it looks. The value was 150 until 8 August — a guess made when the virtualization landed,
which nothing in the corpus was long enough to expose. On a 2,388-message thread it made
the spacer 358,000px against ~193,000px of real content, so the strategy positioned every
item as though it occupied 150px, the items stacked at ~81px, and the rendered block ran
out about halfway down its slot. On screen that was blank space that read as the end of the
conversation. `virtual-scroll.spec.js` now measures rendered bubble heights against the
reserved height and fails if a restyle moves them apart, because the enclave is the wrong
...
- **Stores** hold all client state, in-memory only (the evaluated demo's performance
  ...
  - `SessionStore` — cached machine translations and entities (enrichment is
    ...
    **conversation notes keyed by threadId** (a judgement about the whole exchange, so it
    survives scrolling a 2,000-message stream and leads the gold copy rather than being
    attributed to one message), doc notes, and doc tags with the folded
    `tag → {threads, docs}` index that feeds
...
### The OCR line contract pairs inside the object, on purpose

`/api/ocr` returns `blocks: [{ text, en, bbox }]` — each line's English travels **on the
line**, not in a parallel array. That is not a style choice. A vision model transcribing a
form merges wrapped lines and splits table cells, so `lines[]` and `en[]` come back with
different lengths and every row below the drift point pairs the wrong translation with the
wrong source. Nothing looks broken on screen, and an officer certifies a pairing the
machine never made. One object per line makes the mismatch impossible to express.

`providers/model-gateway.js` normalises whatever the gateway sends: bare strings from an
older prompt still transcribe and simply carry no `en`, and the viewer falls back to the
whole-document `englishGloss` rather than rendering an empty column. Pairing is
all-or-nothing per page for the same reason. `tests/node/ocr-fixtures.test.js` holds the
fixtures to the same guarantee, so the mock cannot promise something the enclave cannot do.

### A bulk verdict is not a read verdict

`TranslationReview.bulk` marks a verdict that came from *Accept remaining* rather than from
opening the message. The gold copy renders those as `bulk-accepted`, never
`linguist-confirmed`. Dropping that flag would make the transcript claim an officer read
text they never opened — the failure `hcd/one_output_model.md` removed the queue-side "mark
reviewed" to avoid.
