...
| 2 | Sat 8 Aug | 5.0 | 2.5 | Internal review round 2 (Tyla, both cuts): Gold Copy export dialog overflowed its own border and maximize was capped — both fixed and regression-tested; walkthrough re-captured and re-narrated to remove a jarring un-maximize before close. Capture now records the clipboard success path the narration describes, and the webm→mp4 step the pipeline had always required by hand is scripted. Shared promote helper removed an order-dependent flake in the suite. 55 e2e green |
