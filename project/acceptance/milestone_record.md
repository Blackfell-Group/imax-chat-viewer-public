...
> and therefore incorporated. River Hawk will re-score against Attachment 2 on receipt if it
> differs. Attachment 3 — the reporting template Article XV(n) and XV(o) both name — is
> likewise referenced but absent.
...
| M2 — repo scaffolded; five mock services containerized and smoke-deployed | 4 | Mon 10 Aug | ✅ complete — run in Platforma |
...
| M5 — all six data deliverables transmitted; live demonstration provided | 14 | **Thu 20 Aug** | ▶ this transmittal |
...
**Status ✅.** The prototype has been **deployed and run in Platforma**, ahead of this
milestone's Day 4 date. The ArgoCD Application view capture evidencing the sync and the
running workloads is attached to the **high-side repository**, which is where it is
producible: it is generated inside the enclave and River Hawk has no path to carry it
low-side into this package. The Government verifies M2 against that capture.

Independently of the environment, the TDD's stated fallback — *"matched local K8s if
sandbox access is pending"* — is also satisfied continuously and mechanically: CI re-proves
the full chain on every push (build → trivy CRITICAL gate → deploy → rollout →
in-cluster smoke). So M2 is evidenced twice, by different means.
...
| **72 e2e specs green**, plus 10 Angular unit and 93 Node unit specs | `npm run test:ng`, `npm test --prefix angular`, `npm run test:node` |
...
**Thursday 20 August 2026.** All six data deliverables are transmitted with this package;
the live demonstration is provided in person on the same date. Transmittal and demonstration
are both required and neither substitutes for the other, so this milestone closes on the
Government's receipt of the package and attendance at the demonstration — recorded here
rather than asserted by the Performer.

The narrated walkthrough is delivered alongside the demonstration as the standing record of
the working product, and as the fallback should enclave access be unavailable on the day.
...
| **D6** | Demo & Acceptance Package — recorded walkthrough (Sponsor receives unlimited rights), summary slide deck (objectives, test results, open issues), live demonstration | MP4 / PDF + live | D14 | `project/acceptance/RiverHawk_IMAX_Walkthrough_2026-08-08.mp4` (4:51, narrated) · `project/delivery/acceptance_deck.pdf` · live demonstration 20 Aug | ✅ recording and deck delivered; the live demonstration is provided on the day |
...
|---|---|---|---|
| FOCI package — SF 328, Key Management Personnel list, Organizational Entity Structure | XIV(c) | Not stated in the agreement | Identified; not submitted — disposition for the AO to direct |
| Cleared Personnel Certification Report (Attachment 3 template) | XV(n) | 30 September annually | Identified; falls due after this period of performance — nil return expected |
| Cleared Personnel Disposition Report (Attachment 3 template) | XV(o) | Within 60 days of completion (~19 October 2026) | Identified; falls due after this period of performance — nil return expected |

Article XV(a) scopes the security article to *"the extent that this Agreement involves
access to national security information."* All work under this agreement is unclassified,
no personnel are cleared under it, and the reports at XV(n) and XV(o) both fall due after
the period of performance ends. River Hawk therefore expects nil returns on all three and
has not submitted them with this package.

That expectation is stated rather than acted on. A nil return is still a return, and which
of these the Agreements Officer wants — and in what form — is the Government's call, not
the Performer's assumption. River Hawk will submit whatever the AO directs, on the schedule
the AO sets. **Attachment 3, which XV(n) and XV(o) both name as the reporting template, is
referenced by the agreement but was not included with it**; the template is needed before
either report can be submitted in the required form.
