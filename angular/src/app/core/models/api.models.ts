...
/**
 * Present when the live enrichment service was configured, attempted, and
 * failed — so the payload is fixture content. Falling back keeps the bench
 * usable during a model outage, but an officer certifying a translation is
 * entitled to know it did not come from the gateway.
 */
export interface ServiceDegraded {
  reason: string;
  attempts: number;
  attempted: string;
}
...
interface ServiceEnvelope {
  ...
  service: string;
  /** Set only when a configured live service was tried and failed (see above). */
