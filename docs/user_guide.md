...
oldest-first, with the `N/M worked` progress bar — plus corpus-wide search when you
...
3. **Add notes where the machine can't.** Code words, source correlation, context — the
   ...
   **A note on the conversation itself** is a different thing, and has its own place.
   *Add conversation note* in the workflow strip records your read on the exchange as a
   whole — "this doesn't sound like a native Russian speaker", "these two clearly know
   each other" — the judgement that comes from having read the thread through rather than
   from any single message. It sits above the stream and **leads the gold copy**, above
   the transcript, because it is a conclusion about all of it. Attached to a message it
   would be read as evidence from that message.
...
- **Accept remaining *N*** — on a long conversation only. A standing channel runs to
  thousands of messages, and clearing the tail one click at a time is not a workflow, so
  this accepts the outstanding machine translations in a single action. It tells you how
  many before it does anything, and **it does not claim you read them**: those messages
  are recorded in the gold copy as `bulk-accepted`, not `linguist-confirmed`, so whoever
  receives the transcript can tell the two apart. On a short thread the button is not
  offered at all — there, reading each message *is* the job.
- **Add conversation note** — your read on the exchange as a whole (see the core loop
  above).
...
OCR viewer shows the scan above the extracted text — multi-page documents page both
together — with a whole-document English gloss beneath.

**Each line carries its own English, beside it.** The extracted text is numbered line by
line, and every line is paired with its translation on the same row, so you read across one
line instead of hunting between two blocks. **The original sits on the side its script
starts from** — left for Russian and Chinese, right for Arabic and Farsi — so a scan always
reads outward from the margin it occupies on the page. A tabular form, where the manifest
rows matter more than the prose, is where this earns its keep.

**Make it big enough to read.** The viewer maximizes to nearly the full screen, resizes from
its bottom-right corner, and the divider between the scan and the text below it drags up and
down, so you can give either one more of the window. Zoom offers **fit page width, fit whole
page, actual size and 200%**, and a zoomed scan pans inside its pane. A dense form — the
...
badge). Entity mode hunts detected entities — people, places, phone numbers and passports —
and also matches sender handles, so a selector query finds the account as well as the
number. Geo-fence and watchlist **groups** are one-click scopes; the facet row (Selector,
Passport, Person, Geo, Image, Unenriched) narrows to messages carrying that signal; the
date range composes with everything. **Flagged** appears with a count once you have handed anything to
...
bar, and content-type lanes with corpus counts. Sorting offers oldest-first, most recent,
message count and flagged-first. (Untranslated-first was retired on 1 August when threads
began arriving already translated — there is no untranslated pile left to lift.) Review edges
