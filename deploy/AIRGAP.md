...
9. **The certificate secrets, and the role that reads them.** (§10, option B) We need the
...
8. **Does the acceptance suite need to run inside?** The 32-spec Playwright suite
...
**This is the target environment path.** the target environment is an automated Kubernetes environment on AWS
the target AWS partition: nobody runs `kubectl create secret`, and the certificates live in Secrets Manager
