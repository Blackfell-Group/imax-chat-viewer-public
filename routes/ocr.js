...
router.post('/', async (req, res) => {
  ...
  if (provider.enabled()) {
    try {
      ...
      return res.json({
        schemaVersion: '1.0',
        service: provider.serviceLabel('vision'),
        engine: provider.serviceLabel('vision'),
        attachmentId,
      ...
      degraded = {
        ...
        attempted: provider.serviceLabel('vision'),
      };
    }
  }

  res.json(degraded ? { ...fixture, degraded } : fixture);
});
